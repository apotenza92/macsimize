...
- Appcasts remain checksum-sealed workflow artefacts. After the protected public-release approval succeeds, the tag-only workflow verifies the seal and publishes only those exact four feed files to the default branch; downstream release work must depend on that publication. Homebrew uses the common attested release bundle and a protected dispatch-only GitHub App credential. The source workflow dispatches Homebrew publication without waiting and never writes the tap. The tap reports and retries publication independently.
